...
function Home() {
	...
	const bigHomeMatch = useMatch("/browse/:Id");
	...
	const { data: rank, isLoading: rankLoading } = useQuery<IGetResult>(
		"rank",
		() => getRankMovie(1)
	);
	...
	const incraseRank = () => {
		if (rank) {
			if (leaving) return;
			toggleLeaving();
			const totalMovies = rank.results.length - 1;
			const maxIndex = Math.floor(totalMovies / offset) - 1;
	...
	const clickedMovieTv = bigHomeMatch?.params.Id && +bigHomeMatch.params.Id;
	...
	return (
		<Wrapper>
			{popularLoading ? (
				...
				<>
					<Banner
						bgPhoto={makeImagePath(popular?.results[7].backdrop_path || "")}
					>
						<Title>{popular?.results[7].title}</Title>
						<Overview>{popular?.results[7].overview}</Overview>
					</Banner>
						...
						{clickedMovieTv ? (
							<>
								...
								>
									{clickedMovieTv && (
										<>
											<DetailMovies movieId={clickedMovieTv} />
										</>
									)}
								</BigMovie>
							</>
